        ...
        {data.map((item,index)=> (
            <Pressable style={{ margin:10 }} key={index}>
              <ImageBackground imageStyle={{borderRadius:6}} style={{aspectRatio:5/6,height:170}} source={{uri:item.image}}>
                <Text style={{position:'absolute',bottom:10,left:10,fontSize:28,fontWeight:'900',color:"white"}}>{item.offer} OFF</Text>
              </ImageBackground>
              <Text style={{marginTop:10,fontSize:17,fontWeight:'700'}}>{item.name}</Text>
